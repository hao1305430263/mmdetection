...
class my_CustomDataset(Dataset):
    ...
    def load_annotations(self, ann_file0, ann_file1):
        ...
        self.cat1label = {cat_id: i for i, cat_id in enumerate(self.cat_ids)}
    ...
    def _filter_imgs(self, min_size=32):
        ...
        for i, [img_info0, img_info1] in enumerate(self.data_infos):
            if (
                min(img_info0["width"], img_info0["height"]) >= min_size
                and min(img_info1["width"], img_info1["height"]) >= min_size
            ):
                valid_inds.append(i)
        return valid_inds
